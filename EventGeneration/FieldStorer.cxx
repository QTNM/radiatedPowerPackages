...
TVector3 rad::FieldStorer::GetInterpolatedEField(double timeInterp)
{
  ...
  // Get the interpolation index
  int interpIndex{GetInterpolationIndex(timeInterp)};
  if (interpIndex == 0)
  {
    timeVals.at(0) = 0;
    eXVals.at(0) = 0;
  ...
  {
    timeVals.at(0) = tA.at(interpIndex - 1);
    eXVals.at(0) = eField.at(interpIndex - 1).X();
    eYVals.at(0) = eField.at(interpIndex - 1).Y();
    eZVals.at(0) = eField.at(interpIndex - 1).Z();
  }
  timeVals.at(1) = tA.at(interpIndex);
  timeVals.at(2) = tA.at(interpIndex + 1);
  timeVals.at(3) = tA.at(interpIndex + 2);
  eXVals.at(1) = eField.at(interpIndex).X();
  eYVals.at(1) = eField.at(interpIndex).Y();
  eZVals.at(1) = eField.at(interpIndex).Z();
  eXVals.at(2) = eField.at(interpIndex + 1).X();
  eYVals.at(2) = eField.at(interpIndex + 1).Y();
  eZVals.at(2) = eField.at(interpIndex + 1).Z();
  eXVals.at(3) = eField.at(interpIndex + 2).X();
  eYVals.at(3) = eField.at(interpIndex + 2).Y();
  eZVals.at(3) = eField.at(interpIndex + 2).Z();
...
TVector3 rad::FieldStorer::GetInterpolatedBField(double timeInterp)
{
  ...
  {
    timeVals.at(0) = tA.at(interpIndex - 1);
    bXVals.at(0) = bField.at(interpIndex - 1).X();
    bYVals.at(0) = bField.at(interpIndex - 1).Y();
    bZVals.at(0) = bField.at(interpIndex - 1).Z();
  }
  timeVals.at(1) = tA.at(interpIndex);
  timeVals.at(2) = tA.at(interpIndex + 1);
  timeVals.at(3) = tA.at(interpIndex + 2);
  bXVals.at(1) = bField.at(interpIndex).X();
  bYVals.at(1) = bField.at(interpIndex).Y();
  bZVals.at(1) = bField.at(interpIndex).Z();
  bXVals.at(2) = bField.at(interpIndex + 1).X();
  bYVals.at(2) = bField.at(interpIndex + 1).Y();
  bZVals.at(2) = bField.at(interpIndex + 1).Z();
  bXVals.at(3) = bField.at(interpIndex + 2).X();
  bYVals.at(3) = bField.at(interpIndex + 2).Y();
  bZVals.at(3) = bField.at(interpIndex + 2).Z();
...
TVector3 rad::FieldStorer::GetInterpolatedPosition(double timeInterp)
{
  ...
  if (interpIndex == 0)
  {
    ...
    zVals.at(0) = pos.at(0).Z();
  }
  else
  {
    timeVals.at(0) = tA.at(interpIndex - 1);
    xVals.at(0) = pos.at(interpIndex - 1).X();
    yVals.at(0) = pos.at(interpIndex - 1).Y();
    zVals.at(0) = pos.at(interpIndex - 1).Z();
  }
  timeVals.at(1) = tA.at(interpIndex);
  timeVals.at(2) = tA.at(interpIndex + 1);
  timeVals.at(3) = tA.at(interpIndex + 2);
  xVals.at(1) = pos.at(interpIndex).X();
  yVals.at(1) = pos.at(interpIndex).Y();
  zVals.at(1) = pos.at(interpIndex).Z();
  xVals.at(2) = pos.at(interpIndex + 1).X();
  yVals.at(2) = pos.at(interpIndex + 1).Y();
  zVals.at(2) = pos.at(interpIndex + 1).Z();
  xVals.at(3) = pos.at(interpIndex + 2).X();
  yVals.at(3) = pos.at(interpIndex + 2).Y();
  zVals.at(3) = pos.at(interpIndex + 2).Z();
