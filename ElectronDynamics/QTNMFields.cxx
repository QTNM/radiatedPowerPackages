...
rad::CoilField::CoilField(const double radius, const double current,
                          const double z, const double mu)
    : coilRadius(radius), coilCurrent(current), coilZ(z), coilMu(mu) {}
...
double rad::CoilField::on_axis_field(const double z) {
  double field = coilMu * coilCurrent * pow(coilRadius, 2) / 2.0 /
                 pow(coilRadius * coilRadius + pow(z - coilZ, 2), 1.5);
  return field;
}
...
TVector3 rad::CoilField::evaluate_field_at_point(const TVector3 vec) {
  ...
  double b_r =
      b_central *
      (int_e * ((1.0 + rad_norm * rad_norm + z_norm * z_norm) / gamma) -
       int_k) /
      root_alpha_pi * (z_rel / rad);
  double b_z =
      b_central *
      (int_e * ((1.0 - rad_norm * rad_norm - z_norm * z_norm) / gamma) +
       int_k) /
...
rad::BathtubField::BathtubField(const double radius, const double current,
...
double rad::SolenoidField::on_axis_field(const double z) {
  ...
  double field =
      (mu * n * i / 2) * ((xiPlus / sqrt(xiPlus * xiPlus + r * r)) -
                          (xiMinus / sqrt(xiMinus * xiMinus + r * r)));
  return field;
}
...
TVector3 rad::SolenoidField::evaluate_field_at_point(const TVector3 vec) {
  ...
  double Br =
      ((2 - kPlus * kPlus) / (2 * kPlus) * int_kPlus - int_ePlus / kPlus) -
      ((2 - kMinus * kMinus) / (2 * kMinus) * int_kMinus - int_eMinus / kMinus);
  Br *= premultR;
...
TVector3 rad::InhomogeneousBackgroundField::evaluate_field_at_point(
    const TVector3 vec) {
  const double r = sqrt(vec.X() * vec.X() + vec.Y() * vec.Y());
  // First calculate the field at the off-axis position r, at z = 0
  const double maxB =
      ((inhomRad * BCent) / pow(inhomRadRadius, 2)) * r * r + BCent;
...
rad::InhomogeneousBathtubField::InhomogeneousBathtubField(
    const double radius, const double current, const double Z,
    const double centralField, const double fractionalInhomZ,
    const double fractionalInhomR) {
  ...
  bkgField = InhomogeneousBackgroundField(centralField, fractionalInhomZ, Z,
                                          fractionalInhomR, radius);
}

TVector3 rad::InhomogeneousBathtubField::evaluate_field_at_point(
    const TVector3 vec) {
  TVector3 field1 = coil1.evaluate_field_at_point(vec);
  TVector3 field2 = coil2.evaluate_field_at_point(vec);
