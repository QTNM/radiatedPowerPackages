...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < grNoise->GetN(); i++) {
    double noiseRoot =
        sqrt(grNoise->GetPointY(i) * bandwidth / (deltaT * 0.5 * N));
  ...
  fCDF->SetTitle(
      "CDF: Rayleigh distribution with T = 4 K, B = 18.5 kHz; x; f(x; #sigma)");
  fCDF->GetHistogram()->GetXaxis()->SetTitle("#sqrt{k_{B}TB} [W^{0.5}]");

  TF1* fPDF = new TF1("fPDF", RayleighPDFFunc, 0, 4 * sigma,
                      1);  // rayleighNorm, 0, 25e-27, 1);
  fPDF->SetParameter(0, sigma * sqrt(0.5));
  fPDF->SetTitle(
      "PDF: Rayleigh distribution with T = 4 K, B = 18.5 kHz; x; f(x; #sigma)");
  ...
  auto hNoiseSqrtPower{std::make_unique<TH1D>(
      "hNoiseSqrtPower",
      "#sqrt{P} in 18.5 kHz bins; #sqrt{P} [W^{0.5}]; N_{bins}", 70, 0, 5e-9)};
  SetHistAttr(hNoiseSqrtPower);
  auto hNoisePower{std::make_unique<TH1D>(
      "hNoisePower", "P in 18.5 kHz bins; P [W]; N_{bins}", 70, 0, 2.5e-17)};
