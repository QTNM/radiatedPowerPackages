...
double GetEModeNormFactor(RectangularWaveguide *wv, int m, int n, double freq, int nSurfPnts)
{
  double area{ (wv->GetLongDimension()/double(nSurfPnts))*(wv->GetShortDimension()/double(nSurfPnts)) };
  double sum{ 0.0 };

  for (int ix = 0; ix < nSurfPnts; ix++) {
    double thisx{ -wv->GetLongDimension()/2.0 + wv->GetLongDimension()/(2.0*double(nSurfPnts)) + wv->GetLongDimension()*double(ix)/double(nSurfPnts) };
    for (int iy = 0; iy < nSurfPnts; iy++) {
      double thisy{ -wv->GetShortDimension()/2.0 + wv->GetShortDimension()/(2.0*double(nSurfPnts)) + wv->GetShortDimension()*double(iy)/double(nSurfPnts) };
      TVector3 surfacePos{ thisx, thisy, 0.0 };
      ComplexVector3 eTransReal{ wv->GetNormalisedEField(RectangularWaveguide::kTE, m, n, surfacePos, freq*2*TMath::Pi()) };
      eTransReal.SetZ(0.0);
...
double GetHModeNormFactor(RectangularWaveguide *wv, int m, int n, double freq, int nSurfPnts)
{
  double area{ (wv->GetLongDimension()/double(nSurfPnts))*(wv->GetShortDimension()/double(nSurfPnts)) };
  double sum{ 0.0 };
  ...
  for (int ix = 0; ix < nSurfPnts; ix++) {
    ...
    for (int iy = 0; iy < nSurfPnts; iy++) {
      double thisy{ -wv->GetShortDimension()/2.0 + wv->GetShortDimension()/(2.0*double(nSurfPnts)) + wv->GetShortDimension()*double(iy)/double(nSurfPnts) };
      TVector3 surfacePos{ thisx, thisy, 0.0 };
      ComplexVector3 hTransReal{ wv->GetNormalisedHField(RectangularWaveguide::kTE, m, n, surfacePos, freq*2*TMath::Pi()) };
      hTransReal.SetZ(0.0);
...
//////////////////////////// Field amplitudes /////////////////////////////////////////
std::complex<double> GetPositiveAmp(RectangularWaveguide *wv, int m, int n, double freq,
				    TVector3 pos, TVector3 vel)
{
  double waveImp{ wv->GetModeImpedance(RectangularWaveguide::kTE, m, n, freq*2*TMath::Pi()) };
  TVector3 j{ -TMath::Qe()*vel };
  ComplexVector3 jComplex{ j };

  ComplexVector3 eTrans{ wv->GetNormalisedEField(RectangularWaveguide::kTE, m, n, pos, freq*2*TMath::Pi()) };
  eTrans.SetZ(std::complex<double>{0.0, 0.0});
  ComplexVector3 eAxial{ wv->GetNormalisedEField(RectangularWaveguide::kTE, m, n, pos, freq*2*TMath::Pi()) };
...
std::complex<double> GetNegativeAmp(RectangularWaveguide *wv, int m, int n, double freq,
				    TVector3 pos, TVector3 vel)
{
  double waveImp{ wv->GetModeImpedance(RectangularWaveguide::kTE, m, n, freq*2*TMath::Pi()) };
  TVector3 j{ -TMath::Qe()*vel };
...
double CalculatePowerPlus(RectangularWaveguide *wv, int m, int n, double freq,
			  ...
			  double integralZPos, int nSurfPnts)
{
  ...
  for (int ix = 0; ix < nSurfPnts; ix++) {
    ...
    for (int iy = 0; iy < nSurfPnts; iy++) {
      ...
      ComplexVector3 ETotal{ wv->GetNormalisedEField(RectangularWaveguide::kTE, m, n, surfacePos, freq*2*TMath::Pi()) };
      ETotal *= aPlus;
      ComplexVector3 HTotal{ wv->GetNormalisedHField(RectangularWaveguide::kTE, m, n, surfacePos, freq*2*TMath::Pi()) };
      HTotal *= aPlus;
...
int main(int argc, char *argv[])
{
  ...
  const double centralBField{ field->evaluate_field_at_point(TVector3(0, 0, 0)).Mag() };
  const double centralFreq{ 26e9 }; // GHz
  std::cout<<"Central frequency = "<<centralFreq/1e9<<" GHz\n"<<std::endl;

  const double ke{ (TMath::Qe()*centralBField/(2*TMath::Pi()*centralFreq) - ME)*pow(TMath::C(), 2)/TMath::Qe() };
  std::cout<<"KE = "<<ke<<std::endl;
  const double speed{ GetSpeedFromKE(ke, ME) };
  const TVector3 velocity{ speed, 0, 0 };
  const double gyroradius{ GetGyroradius(velocity, field->evaluate_field_at_point(TVector3(0, 0, 0)), ME) };
  ...
  RectangularWaveguide *WR42 = new RectangularWaveguide(WR42Side1, WR42Side2, waveguideLength);
  ...
  double waveImp{ WR42->GetModeImpedance(RectangularWaveguide::kTE, 1, 0, centralFreq*2*TMath::Pi()) };
  std::cout<<"Integral E = "<<integralE<<std::endl; // Should be 1
  std::cout<<"Integral H = "<<integralH<<std::endl; // Should equal to the below
  std::cout<<"1 over wave impedance squared = "<<1.0/(waveImp*waveImp)<<std::endl;  
  ...
  grCollectedPower->SetTitle("Collected power (unscaled): WR42 waveguide; X [mm]; Collected power [W]");
  TGraph *grCollectedPowerCrude = new TGraph();
  setGraphAttr(grCollectedPowerCrude);
  grCollectedPowerCrude->SetMarkerStyle(20);
  grCollectedPowerCrude->SetTitle("Collected power (unscaled): WR42 waveguide; X [mm]; Collected power [W]");
  TGraph *grCollectedFrac = new TGraph();
  setGraphAttr(grCollectedFrac);
  grCollectedFrac->SetMarkerStyle(20);
  grCollectedFrac->SetTitle("Collected power fraction: WR42 waveguide; X [mm]; Fraction of total power");

  for (int iOffset = 0; iOffset < nOffsets; iOffset++) {
    double testOffset{ minOffset + double(iOffset)*(maxOffset-minOffset)/double(nOffsets-1) };
    std::cout<<testOffset*1e3<<" mm"<<std::endl;
  
    // Generate the electron trajectory
    ElectronTrajectoryGen traj(trackFile, field, TVector3(testOffset, -gyroradius, 0.0), TVector3(speed, 0, 0), 1e-12, 1e-9, 0.0, 2 * R_E / (3 * TMath::C()));
    traj.GenerateTraj();
  
    // Now read in the file again and calculate some mode amplitudes as a function of time
    ...
    double area{ (WR42->GetLongDimension()/double(nSurfPnts))*(WR42->GetShortDimension()/double(nSurfPnts)) };

    TGraph *grPowerPlus = new TGraph();
    setGraphAttr(grPowerPlus);
    grPowerPlus->SetTitle(Form("x = %.2f: P^{+}; Time [s]; P^{+}", testOffset*1e3));
    grPowerPlus->SetMarkerStyle(20);
    TGraph *grCrudePlus = new TGraph();
    setGraphAttr(grCrudePlus);
    grCrudePlus->SetTitle(Form("x = %.2f: P^{+}; Time [s]; P^{+}", testOffset*1e3));
    TGraph *grAPlus = new TGraph();
    setGraphAttr(grAPlus);
    grAPlus->SetTitle(Form("x = %.2f: A^{+}; Time [s]; A^{+}", testOffset*1e3));
    ...
    for (int iE = 0; iE < tree->GetEntries(); iE++) {
      ...
      std::complex<double> ampPlus{ GetPositiveAmp(WR42, 1, 0, centralFreq, thePos, theVel) };
      double totalPowerPlus{ CalculatePowerPlus(WR42, 1, 0, centralFreq, thePos, theVel, volumeLength, nSurfPnts) };
      grAPlus->SetPoint(grAPlus->GetN(), time, ampPlus.real());
      grCrudePlus->SetPoint(grCrudePlus->GetN(), time, pow(abs(ampPlus), 2)/waveImp);
    ...
    std::cout<<"Radiated power is "<<radiatedPower*1e15<<" fW"<<std::endl;
    ...
    grCollectedPower->SetPoint(grCollectedPower->GetN(), testOffset*1e3, timeAvgPower);
    grCollectedPowerCrude->SetPoint(grCollectedPowerCrude->GetN(), testOffset*1e3, timeAvgPowerCrude);
    grCollectedFrac->SetPoint(grCollectedFrac->GetN(), testOffset*1e3, timeAvgPower/radiatedPower);
