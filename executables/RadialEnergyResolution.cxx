...
int main(int argc, char *argv[]) {
  ...
  unique_ptr<ComsolField> cfield = std::make_unique<ComsolField>(
      "/home/sjones/work/qtnm/radiatedPowerPackages/files/"
      "fieldmap_50A_170mmx1000mm.csv");
  const double centralFieldComsol{
      cfield->evaluate_field_magnitude(TVector3(0, 0, 0))};
  ...
  for (int iR{0}; iR < nRPnts; iR++) {
    double testRadius{minRadius + (maxRadius - minRadius) * double(iR) /
                                      double(nRPnts - 1)};
    double testField{
        field->evaluate_field_magnitude(TVector3(0, testRadius, 0))};
    grDeltaB->SetPoint(iR, testRadius,
                       (testField - centralField) / centralField);
    double testFieldComsol{
        cfield->evaluate_field_magnitude(TVector3(0.001, testRadius, 0.001))};
    grDeltaBComsol->SetPoint(
        iR, testRadius,
  ...
  TH1D *hB{new TH1D("hB", "B field; B_{init} [T]; N_{electrons}", 100,
                    centralField, centralField * (1 + 0.9e-3))};
  SetHistAttr(hB);
  ...
  THStack *hs = new THStack("hs",
                            "B field resolution as a function of radial "
                            "position resolution; #Delta B / B; N_{e}");
  ...
  for (int n{0}; n < nResPnts; n++) {
    ...
    TH1D *hBRes = new TH1D(Form("hBRes%d", n), Form("#sigma_{R} = %.4f", res),
                           400, -0.0015, 0.0015);
    SetHistAttr(hBRes);
    ...
    TF1 *fSGaus =
        new TF1(Form("fSGaus%d", n), SkewedGaussian_f, -0.002, 0.002, 4);
