/// FFTAnalysis.cxx - ROOT-based FFT and frequency domain analysis (using
/// SignalUtils internally)
...
TGraph *rad::MakePowerSpectrumNorm(const TGraph *grWave) {
  ...
  if (n < 2) {
    throw std::invalid_argument(
        "Input graphs requires at least two points to compute spectrum");
  }
  ...
  TGraph *result = new TGraph(spectrum.first.size(), spectrum.first.data(),
                              spectrum.second.data());
  setGraphAttr(result);
  return result;
}
...
TGraph *rad::MakePowerSpectrumPeriodogram(const TGraph *grWave) {
  ...
  if (n < 2) {
    throw std::invalid_argument(
        "Input graph requires at least two points to compute spectrum");
  }
...
TGraph rad::MakePowerSpectrumPeriodogram(const TGraph &grWave) {
  ...
  // Convert back to TGraph
  TGraph result(spectrum.first.size(), spectrum.first.data(),
                spectrum.second.data());
  setGraphAttr(result);
  result.GetXaxis()->SetTitle("Frequency [Hz]");
  return result;
...
TGraph *rad::BandPassFilter(const TGraph *grWave, const double minFreq,
  ...
  std::vector<double> timeVec(timeData, timeData + n);
  auto filteredData =
      rad::BandPassFilterSpectrum(timeVec, deltaT, minFreq, maxFreq);
