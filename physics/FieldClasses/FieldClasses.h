...
class FieldPoint {
 protected:
  ...
  /// Produces a TGraph of the same variable on the y axis of the input graph
  /// but using the retarded time
  ...
  IAntenna*
      myAntenna;  // The chosen antenna, contains position and direction info
  ...
  /// \param minCutTime The minimum time at which the outputted graph should
  /// start
 ...
 public:
  enum Coord_t { kX, kY, kZ };

  /// Parametrised constructor
  /// \param trajectoryFilePath TString to the input ROOT file containing
  /// electron trajectory
  ...
  TGraph* GetPositionTimeDomain(Coord_t coord,
  ...
  TGraph* GetPoyntingVecTimeDomain(Coord_t coord,
                                   const bool kUseRetardedTime = false);
  ...
  /// Load voltage from the antenna as a function of time (assumes impedance
  /// matching)
  /// \param kUseRetardedTime Boolean to use retarded time
  /// \param firstPoint First point of the class members to return
  ...
  /// \param coord The chosen component
  /// \param kUseRetardedTime Boolean to use retarded time
  TGraph* GetEFieldPowerSpectrumNorm(Coord_t coord,
                                     const bool kUseRetardedTime = false);
